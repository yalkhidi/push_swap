...
/*   Updated: 2025/05/11 17:18:49 by yalkhidi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	char		**pbuffer = get_gnl_buffer(); // <- get the address of the shared static buffer
	char		*line;
	int			endof;
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || BUFFER_SIZE >= INT_MAX)
		return (NULL);
	endof = 1;
	while (!check_end(*pbuffer) && endof != 0)
	{
		*pbuffer = read_to_buff(*pbuffer, fd, &endof);
		if (!*pbuffer)
			return (NULL);
	}
	if (endof == 0 && (*pbuffer)[0] == '\0')
	{
		free(*pbuffer);
		*pbuffer = NULL;
		return (NULL);
	}
	line = extract_line(pbuffer);
	return (line);
}
